...
withDefaults(
  ...
  { kind: 'info', title: undefined, requestId: undefined },
)
 ...
 * convey with hue.
 */
// Keyed by the prop's union rather than by `string`: a `Record<string, T>` lookup is possibly
// undefined, which is honest — it would be, for any key outside the four — and forces a
// non-null assertion in the template. Keying it by AlertKind makes the lookup total instead.
const styles: Record<AlertKind, { wrapper: string; icon: string }> = {
  ...
  info: {
    wrapper: 'bg-info/10 text-info border-info/30',
